...
const Messages = (props: Props) => {
    ...
    return (
        <div className=" px-4 md:px-0 grid  md:grid-cols-3  md:divide-x md:divide-background-foreground min-h-[calc(100vh-6.5rem)]">
            <div className="grid md:col-span-1 h-full pr-1">
                ...
                {messages.data.map((message) => (
                    ...
                    >
                        <div className="flex justify-between items-center">
                            ...
                            <small className="truncate md:w-24 w-20">
                                {format(message.created_at, "dd MMMM yyyy", {
                                    locale: fr,
                                })}
                            </small>
                        ...
                        >
                                ...
                                if (isNaN(labelNumber)) {
                                    ...
                                    return link.label === "&laquo; Previous" && (
                                        <PaginationItem key={index}>
                                            <PaginationPrevious
                                            className={cn(!link.url ? "cursor-default text-muted hover:bg-background hover:text-muted" : "")}
                                                href={link.url}
                                            />
                                        </PaginationItem>
                                    );
                            ...
                            {messages.meta.links.map((link, index) => {
                                ...
                                if (isNaN(labelNumber)) {
                                    ...
                                    return link.label !== "&laquo; Previous" && (
                                        <PaginationItem key={index}>
                                            <PaginationNext href={link.url} className={cn(!link.url ? "cursor-default text-muted hover:bg-background hover:text-muted" : "")} />
                                        </PaginationItem>
                                    );
                                }
                            })}
