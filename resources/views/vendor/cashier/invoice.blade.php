    ...
    <table style="margin-left: auto; margin-right: auto;" width="100%">
        <tr valign="top">
            <td width="160">
                ...
                <p>
                    @isset ($product)
                        <strong>Produit:</strong> {{ $product }}<br>
                    @endisset

                    <strong>Date:</strong> {{ $invoice->date()->toFormattedDateString() }}<br>

                    @if ($dueDate = $invoice->dueDate())
                        <strong>Date d'échéance:</strong> {{ $dueDate->toFormattedDateString() }}<br>
                    @endif

                    @if ($invoiceId = $id ?? $invoice->number)
                        <strong>Numéro de facture:</strong> {{ $invoiceId }}<br>
                    @endif
                </p>
            </td>
            ...
            <td colspan="2">
                ...
                <table width="100%" class="table" border="0">
                    ...
                    @if ($invoice->hasDiscount() || $invoice->hasTax() || $invoice->hasStartingBalance())
                        <tr>
                            <td></td>
                            <td colspan="{{ $invoice->hasTax() ? 3 : 2 }}">Sous-total</td>
